...
shinyServer(function(input, output, clientData, session) {
  ...
  output$submit_map <- renderLeaflet({
    ...
    if (!is.null(rs$current_img_gps)) {
      ...
      for (i in unique(zone_to_plot$group)) {
        ...
        map <- map %>% addPolygons(
          lat = temp$latitude, lng = temp$longitude,
  ...
  observeEvent(input$submit, {
    ...
    con <- dbcon(isLocal)
    temp <- data.frame(
      id = max(dbReadTable(con, "bounties")$id) + 1,
    ...
    if (input$submit_quest %in% active_bounties$id_quest) {
      # Check that data object exists and is data frame.
      sendSweetAlert(session,
        title = lang$bounty_already_title,
      ...
      sendSweetAlert(session,
        title = lang$no_more_quests_title,
      ...
      dbSendStatement(con, statement)
      sendSweetAlert(session,
        title = lang$bounty_submitted, text = NULL, type = "success",
        btn_labels = "Ok", html = FALSE, closeOnClickOutside = TRUE
      )
    }
    dbDisconnect(con)
  ...
  observeEvent(input$reset_password, {
    ...
    dbDisconnect(con)
    sendSweetAlert(session,
      title = "Password reset", text = NULL, type = "success",
      btn_labels = "Ok", html = FALSE, closeOnClickOutside = TRUE
    )
  })
  ...
  observeEvent(input$update_params, {
    ...
    dbDisconnect(con)
    sendSweetAlert(session,
      title = "Parameters changed", text = NULL, type = "success",
      btn_labels = "Ok", html = FALSE, closeOnClickOutside = TRUE
    )
  })
  ...
  observeEvent(input$recompute_group, {
    ...
    sendSweetAlert(session,
      title = "Points recomputed", text = NULL, type = "success",
      btn_labels = "Ok", html = FALSE, closeOnClickOutside = TRUE
    )
  })
  ...
  observeEvent(input$update_table, {
    ...
    # Success alert
    sendSweetAlert(session,
      title = paste0("Table ", input$select_table_1, " updated"),
      text = NULL, type = "success",
